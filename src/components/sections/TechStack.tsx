...
const techItems = [
  ...
  "Node.js", "React Native", "Figma", "PostgreSQL",
];
        ...
        >
          <motion.div className="mb-16" variants={fadeUp}>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-heading">
              {dict.techStack.title}<span className="accent-dot">.</span>
            </h2>
          ...
          <motion.div className="flex flex-wrap gap-4" variants={staggerContainer}>
            {techItems.map((tech, i) => (
              <motion.div
                key={i}
                className="px-6 py-3 border border-themed text-sm text-heading hover:border-accent hover:bg-accent hover:text-white transition-all duration-200 cursor-default"
                variants={fadeUp}
              >
                {tech}
              </motion.div>
